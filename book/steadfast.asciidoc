...
.Abbildung 8. Traefik-Architekturdiagramm, das zeigt, wie eine Anfrage einen Dienst erreicht.Aus dem MIT-lizenzierten Quellcode von Traefik. Danke an Peka für das Gopher-Logo, lizenziert unter CC-BY-3.0.
...
.Traefik-Konfigurationsausschnitt (🏠 admin computer)
...
.Nextcloud-Konfigurationsausschnitt (🏠 admin computer)
...
Siehe auch <<Lösungs-Tauglichkeits-Checkliste>> in <<Alternativen zu mario>>.
...
* Open-Core-Modell ([https://de.wikipedia.org/wiki/Open-Core-Modell](https://de.wikipedia.org/wiki/Open-Core-Modell))
...
[Nicks Erklärung](https://help.nextcloud.com/t/build-bzip-and-package-from-git/58341/2) dafür ist nachvollziehbar:
...
Ich habe auf eBay einen gebrauchten, generalüberholten 1U-Rackmount-Server für etwa 1.000 USD gefunden.
...
Ein 1U-Server ist ein **Rack Unit** hoch – etwa so groß wie eine lange Pizzaschachtel ([Wikipedia-Link](https://de.wikipedia.org/wiki/Rack-Einheit)).
...
[#image-racked-server]
.Selbstgebauter Rackmount-Server, an der Garagendecke montiert.Macht Spaß, ihn anzuschauen, und er ist aus dem Weg – aber für Wartungsarbeiten brauche ich eine Leiter, und er wiegt etwa 23 kg.
...
(((ZFS, snapshots and)))
Ich verwende ((Festplattenlaufwerke (HDDs))) zur Datenspeicherung – hauptsächlich, um Kosten gegenüber öffentlichem Cloud-Speicher oder ((SSD))s (Solid-State-Laufwerken) zu sparen.
Die Kosten für ((Block Storage)) in der Public Cloud übersteigen bei Weitem die Gigabyte-Stunden-Kosten meiner Festplattenlaufwerke.
Ich habe den Preis für einen Monat 5 TB Festplattenlaufwerke-Blockspeicher bei AWS auf 228,10 $ geschätzt.
Mit ZFS erstelle ich außerdem alle 15 Minuten einen Snapshot (im Grunde ein vollständiges lokales Backup).
Ein Monat stündlicher Snapshots (das nächstvergleichbare Angebot, das ich finden konnte) kostet weitere 310,68 $ bei AWS.
...
Festplattenlaufwerke (HDDs) sind völlig ausreichend schnell, wenn man die Antwortzeiten selbstgehosteter Dienste als Maßstab nimmt. Das Betriebssystem und die Dienste arbeiten effizient mit Daten-Caching, sofern der Server über ausreichend RAM verfügt.
...
Hier ist ein einfaches Diagramm, das ich mit [https://asciiflow.com](https://asciiflow.com) erstellt habe, um die Verkabelung zu planen und den Datenfluss durch meine Netzwerkgeräte zu visualisieren:
...
=== Betriebssystem-Installation (OS-Installation)
...
Der Code unten geht davon aus, dass du mit einer Festplatte für das Betriebssystem angefangen hast, das OS diese als `/dev/sda` erkannt hat,
...
Wenn du es dir leisten kannst, ist der einfachste Weg, einen Server zuverlässig zu betreiben, *zwei identische Server zu kaufen*.
...
(((Borg)))
(((restic)))
Ich empfehle eine Backup-Strategie, die ZFS-Snapshots mit entweder [https://restic.net\[restic](https://restic.net[restic)] oder [https://borgbackup.org\[Borg](https://borgbackup.org[Borg)] kombiniert, um die Daten extern zu sichern.
[https://reddit.com/r/BorgBackup/comments/v3bwfg/\[Hier](https://reddit.com/r/BorgBackup/comments/v3bwfg/[Hier) ist ein guter Vergleich zwischen restic und Borg].
...
Es ist im Wesentlichen ein Wrapper um das bewährte [https://ansible.com\[Ansible\]-Systembereitstellungstool](https://ansible.com[Ansible]-Systembereitstellungstool).
...
Erstelle _auf deinem Server_ den Ordner: ~/ping/.
...
Drücke nach ein paar Sekunden Strg + C.
