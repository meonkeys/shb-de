...
Sobald Sie Nextcloud zum Laufen gebracht haben, finden Sie unter <<Mehr zu Nextcloud>> viele meiner Gedanken darüber, wie man es am besten anpasst.

[#nextcloud_quick_start]
==== Schnellstart

. Provisionieren Sie mit mario von Ihrem Admincomputer.
. Starten Sie Nextcloud mit `dc nextcloud up -d` auf Ihrem Server.
. Rufen Sie `\https://cloud.example.com` auf Ihrem Admincomputer auf.
...
Wenn Sie über eine Grafikprozessor verfügen, sollten Sie sich die Hardwarebeschleunigung ansehen.
Wenn Sie über eine Grafikprozessor verfügen, sollten Sie sich die https://jellyfin.org/docs/general/administration/hardware-acceleration/[Hardwarebeschleunigung ansehen].
...
(((Single sign-on)))
Es wäre für Benutzer sehr praktisch, sich nur einmal einloggen zu müssen, um Zugriff auf alle selbstgehosteten Dienste über einen gemeinsamen, konsistenten und gut gestalteten Mechanismus zu erhalten (Single Sign-On).
Für Systemadministratoren wäre es ebenso hilfreich, alle Benutzer und Gruppen zentral an einer Stelle verwalten zu können (zentrale Identitätsverwaltung).
...
Führen Sie es mit `dc nextcloud exec --user www-data app php occ` aus.

Fügen Sie `/data/video` als externen Speicher hinzu.
Medien, die dort hochgeladen werden, erscheinen automatisch in Jellyfin.
Besuchen Sie zuerst `/settings/apps/featured` und installieren Sie die App „External storage support“.
Besuchen Sie anschließend `/settings/admin/externalstorages` und installieren Sie die App „External storage support“.
...
Fügen Sie `/data/music` als externen Speicher hinzu, ähnlich wie `/data/video`.

==== Weitere Wartungstipps

Besuchen Sie regelmäßig `/settings/admin/overview`, um nach System- und Sicherheitsproblemen zu suchen, die möglicherweise manuelles Eingreifen erfordern.
Führen Sie alle auf dieser Seite empfohlenen Wartungsmaßnahmen durch.
Ignorieren Sie den Abschnitt „Update“, er kann von Docker Hub abweichen.
Besuchen Sie regelmäßig `/settings/admin/logging`, um alle serverseitigen Protokollmeldungen zu überprüfen.
...
Die Verwendung dieses Tags ist für Sie und Ihre Benutzer wahrscheinlich stabil genug.
`stable` entspricht im Allgemeinen der Version, die unter https://docs.nextcloud.com/ als _previous_ bezeichnet wird.

Sie können Ihre Nextcloud auf eine spezifischere Version wie `27.1.5-apache` „fixieren“.
Das gibt Ihnen die Möglichkeit, jedes Upgrade zu überprüfen und zu testen.
Sie können einen Release-Tag in der `compose.yml` von Nextcloud festlegen, indem Sie dort, wo der Image-Name erscheint, einfach `stable` durch den gewünschten Tag ersetzen.
...
Sie sollte sofort geöffnet werden.
...
Benutzer mit iOS oder anderen Betriebssystemen außer Android können Groupware-bezogene Daten https://docs.nextcloud.com/server/stable/user_manual/en/groupware/[ohne DAVx5] synchronisieren.
...
Der AIO verfolgt einen anderen Ansatz als mario: Er konfiguriert und verwaltet mehrere Nextcloud-bezogene Service-Container für Sie.
Ich empfehle stattdessen die mario-Methode, da sie eine flexible und lehrreiche Erfahrung bietet, bei der Sie lernen, wie Sie einzelne Container selbst hinzufügen und verwalten können.
...
Wenn Sie es eilig haben, können Sie One-Click-Installationen mit vielen sofort einsatzbereiten Apps finden.
(((FreedomBox)))
...
. Wie könnten Sie erkennen, ob Ihr Server kompromittiert wurde?
...
Sie orientieren sich so weit wie möglich an der gebräuchlichen Verwendung.
...
((UI))::
  User Interface – Benutzeroberfläche. Die Schnittstelle zwischen Benutzer und System, z.B. eine Website oder App.Wird oft zusammen mit der Benutzererfahrung betrachtet und als „UI/UX“ bezeichnet.
((USV))::
...
((UX))::
  User Experience – Benutzererlebnis. Beschreibt die Art der Interaktion zwischen Benutzer und System, einschließlich Bedienfreundlichkeit und der Schritte, die nötig sind, um eine Aufgabe zu erfüllen. Wird häufig zusammen mit der Benutzeroberfläche betrachtet („UI/UX“).
((Volume))::
...
((ZFS))::
  Dateisystem mit vielen erweiterten Funktionen wie Verschlüsselung, Schutz vor Bit Rot, Journaling, Volume-Management und Snapshots. Ursprünglich Abkürzung für „Zettabyte File System“.
