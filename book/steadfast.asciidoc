...
Grafikprozessor-Arbeitslasten werden in diesem Buch nicht behandelt, obwohl <<_whats_next>> und <<Exercises>> einige Dinge ansprechen, die du selbst ausprobieren kannst.
...
[#image-service-stack]
.Schichten eines Steadfast systems. Von unten, Hardware: bare Metal, Dateisystemsystem: ext4 for / and optionally ZFS for /data, Betriebssystem: Ubuntu LTS 64-bit server, Containerlaufzeit: Docker, Container: Nextcloud Dateifreigabe, Jellyfin Medienserver, Wallabag Artikelleser.
...
* Bei Problemen lassen sich über Websuche leicht Informationen finden (z. B. bekannte Probleme oder Problemumgehung)
...
Ich habe auf eBay einen gebrauchten, generalüberholten 1HE-Gehäusestapel für etwa 1.000 USD gefunden.
...
Mein Server hat zwei 24-Kern-CPUs und 128 GB RAM.

[#image-racked-server]
.Selbstgebauter Gehäusestapel-Server, an der Garagendecke montiert.Macht Spaß, ihn anzuschauen, und er ist aus dem Weg – aber für Wartungsarbeiten brauche ich eine Leiter, und er wiegt etwa 23 kg.
...
Der durchschnittliche Stromverbrauch liegt bei 130 W, also etwa 1.140 kWh pro Jahr – das entspricht ungefähr 138,15$ in Seattle.
Das ist in etwa so viel wie eine helle Glühlampe verbraucht, und für eine einzelne Person etwas verschwenderisch.
Aber bei fünf Nutzer*innen? Etwa 228 kWh pro Jahr und Person.
...
Ich habe den Preis für einen 5 TB Blockspeicher bei AWS auf 228,10 $ pro Monat geschätzt.
...
Falls beim Aufruf deiner Webdienste Zertifikatswarnungen erscheinen, prüfe zuerst die Traefik-Logs wie in <<Start Reverse-Proxy>> beschrieben.
...
|Programmieren mit visuellen Werkzeugen lernen |<<Scratch: visual programming>>
|===
...
Es wäre großartig, wenn man [https://features.jellyfin.org/posts/1036/bookmark-audio-video-segments\[Clips](https://features.jellyfin.org/posts/1036/bookmark-audio-video-segments[Clips) erstellen] könnte, ohne tatsächlich neue Mediendateien erstellen zu müssen.
...
Oder verwende die Konfigurationseinstellungen, um automatische Updates für bestimmte Container zu erlauben oder zu blockieren.
...
Ich wollte viele zusätzliche Details einfügen, ohne <<Services>> zu überladen, daher findest du diese zusätzlichen Abschnitte hier.
...
Meine klare Präferenz liegt bei Collabora, im Einklang mit <<GGeeignet fürs Self-Hosting>>; trotz weniger GitHub-Sterne scheint die Entwicklung von Collabora zu florieren, während ONLYOFFICE stagniert (obwohl schwer zu sagen ist, welche der vielen ONLYOFFICE-Repositories auf GitHub hier relevant sind).
